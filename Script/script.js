...
function showData(){
    ...
    for (let i =0; i<container.length;i++){
        ...
        <td> <p
                ...
                type="text" class="form-control d-none "
                id="descUpdate"></td>
        <td><button onclick="deleteData(${i})" class="btn btn-outline-danger"id='deletebtn'>Delete</button></td>
...
function search(value) {
    ...
    for (let i =0; i<container.length;i++){
        ...
        <td> <p
                ...
                type="text" class="form-control d-none "
                id="descUpdate"></td>
        <td><button onclick="deleteData(${i})" class="btn btn-outline-danger"id='delbttn'>Delete</button></td>
        <td><button onclick="updateData(${i})" class="btn btn-outline-warning"id='updatebttn'>Update</button>
        <button class="btn btn-outline-success px-3 show"id='savebtn'>Save</button></td>
    </tr>
        `
...
let updateButton = document.getElementById('updatebttn');
...
createUser.addEventListener('click',function(e) {
    ...
    dataOfUsers.push(users);
    localStorage.setItem('users', JSON.stringify(dataOfUsers));
    ...
    if (users.userRole === "user") {
        ...
        updateButton.style.display ="none";
              // Hide all elements with class name 'updatebttn'
       let buttons = document.querySelectorAll('#deletebtn');
       for (let button of buttons) {
           button.style.display = "none";
       }
       let buttons2 = document.querySelectorAll('#updatebttn');
